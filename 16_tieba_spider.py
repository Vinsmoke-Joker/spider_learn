...
class TiebaSpider(object):
    def __init__(self):
        self.tieba_name = input('输入要爬取的贴吧名字:')
        ...
        self.start_url = "http://tieba.baidu.com/mo/q----,sz@320_240-1-3---2/m?kw="+self.tieba_name+"&pn=0"
        # self.start_url = "https://tieba.baidu.com/mo/q---162472c26443a37ff6a3806493e4121c:FG =1-sz@320_240,,-2-3-0--2/m?kw="+self.tieba_name+"&lp=7202&referer=tieba.baidu.com&pn=0&"
        self.part_url = "http://tieba.baidu.com/mo/q----,sz@320_240-1-3---2/"

    def parse_url(self,url): # 2.发送数据，请求响应
        print(url)
    ...
    def get_img_list(self,detail_url,total_img_list): # 获取帖子的图片
        # 3.2 请求列表页url地址，获取详情页的第一页
        detail_html_str = self.parse_url(detail_url)
        detail_html = etree.HTML(detail_html_str)
        # 3.3 提取详情页第一页图片，提取下一页地址
        img_list = detail_html.xpath("//img[@class='BDE_Image']/@src")
        total_img_list.extend(img_list)
        # 3.4 请求详情页下一页地址，循环3.2-3.4步
        ...
        return total_img_list  # if 中存在return 可以省略else

    def save_content_list(self,content_list):
        file_path = './data/tieba/'+self.tieba_name+'.txt'
